...
public final class ViewMatchers {
  ...
  public enum Visibility {
     ...
     */
    public static Visibility forView(View view) {
  ...
  static final class HasSiblingMatcher extends TypeSafeDiagnosingMatcher<View> {
    ...
    protected boolean matchesSafely(View view, Description mismatchDescription) {
      ...
      mismatchDescription
          ...
          .appendText(" siblings didn't match");
      return false;
    }
  }
  ...
  static final class IsDescendantOfAMatcher extends TypeSafeDiagnosingMatcher<View> {
    ...
    @Override
    public void describeTo(Description description) {
      description.appendText("has ancestor matching ").appendDescriptionOf(ancestorMatcher);
    }

    @Override
    protected boolean matchesSafely(View view, Description mismatchDescription) {
      int parentCount = checkAncestors(view.getParent(), 0);
      if (parentCount != 0) {
        mismatchDescription
            .appendText("all ")
            .appendValue(parentCount)
            .appendText(" ancestors didn't match ")
    ...
    /**
     * @param viewParent The {@link ViewParent} to test.
     * @param count The running count of parents checked.
     * @return 0 for a match; otherwise, the number of parents that were checked.
     */
    private int checkAncestors(ViewParent viewParent, int count) {
      ...
      return checkAncestors(viewParent.getParent(), count + 1);
    }
  }
      ...
      if (visibility.getValue() == View.VISIBLE) {
        if (view.getVisibility() != visibility.getValue()) {
          mismatchDescription
              ...
              .appendValue(Visibility.forView(view));
          return false;
        }
